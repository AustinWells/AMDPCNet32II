...
- (void)setupInitBlock {
  ...
  IOLog("AMDPCNet32II: initBlock Address %x\n", (unsigned int)initBlock);
  // shitty DIY alignment
  for (i = 0; i < 4; i++) {
    if ((((unsigned int)initBlock) + i) % 4 == 0) {
      initBlock = initBlock + i;
      break;
    }
  }
  IOLog("AMDPCNet32II: Virtual : Aligned initBlock Address %x\n",
        (unsigned int)initBlock);
...
- (void)startCard {
  ...
  long csr0;
  [self writeCSR32:LANCE_CSR0:(LANCE_CSR0_INIT | LANCE_CSR0_STRT)];
...
- (void)interruptOccurred {
  ...
  if (csr3 & LANCE_CSR3_TINTM) { IOLog("AMDPCNet32II: TINT mask enabled\n"); }
  else { IOLog("AMDPCNet32II: TINT mask disabled\n"); }

  if (csr3 & LANCE_CSR3_RINTM) { IOLog("AMDPCNet32II: RINT mask enabled\n"); }
  else { IOLog("AMDPCNet32II: RINT mask disabled\n"); }

  if (csr3 & LANCE_CSR3_IDONM) { IOLog("AMDPCNet32II: IDON mask enabled\n"); }
  else { IOLog("AMDPCNet32II: IDON mask disabled\n"); }
  ...
  [self writeCSR32:LANCE_CSR0:0xf800 | writeBack | LANCE_CSR0_TDMD];
